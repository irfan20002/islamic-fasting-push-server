...
// In-memory subscription store (persists while server runs)
...
app.post('/subscribe', (req, res) => {
  ...
  const id = subscription.endpoint.slice(-20); // unique ID from endpoint
...
function calcPrayerTimes(lat, lon, date) {
  ...
  const ishraqStart = sunrise + 25/60; // 25 min after sunrise
  
  // Last third of night: midpoint between midnight and next fajr
  const nextFajr = noon + fajrHA + 24; // next day fajr approx
  ...
  return {
    ...
    Tahajjud:  toDate(lastThird - 15/60), // 15 min before last third
...
function isFastingDay(date) {
  ...
  // Weekly sunnah: Monday (1) and Thursday (4)
  if (dow === 1 || dow === 4) return { fasting: true, name: dow === 1 ? 'Monday fast' : 'Thursday fast' };
  // Could add Ayyam al-Beed, Ashura etc. here
  ...
  for (const [id, sub] of subscriptions) {
    try {
      ...
      function isNow(t, offsetMins = 0) {
        ...
        return Math.abs(target - nowMin) < 60000; // within 1 minute
      }

      // 🍽️ Suhoor — 30 min before Fajr (fasting days only)
      ...
      // 🌙 Tahajjud — 15 min before last third
